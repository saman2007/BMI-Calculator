...
function displayBMI() {
  ...
  //display loading gif for about 1 second and then show the values
  setTimeout(function () {
    document.querySelector(".loading").style.display = "none";
      //check the values
      if (heightInput.value != "" && weightInput.value != "") {
        //creat an instance with users entered values
        BMI = new CalculateBMI(
          Number(heightInput.value),
          Number(weightInput.value)
        );
        //check calculate returned values
        if (
          BMI.calculate() == "your height and weight is not in range." ||
          BMI.calculate() == "your height is not in range." ||
          BMI.calculate() == "your weight is not in range."
        ) {
          ...
          document.querySelector(".BMI .info .number").textContent =
            BMI.calculate();
          document.querySelector(".BMI .situation span").textContent =
            BMI.situation();
          openModal();
        }
      } else {
        ...
        document.querySelector(".error").textContent =
          "you didnt enter anything or you entered some letters";
        openModal();
      }
  }, 1500);
}
